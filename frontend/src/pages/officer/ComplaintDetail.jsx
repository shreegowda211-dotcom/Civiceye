...
export default function OfficerComplaintDetail() {
  ...
  const update = async (payload) => {
    ...
    qc.invalidateQueries({ queryKey: ["officer-complaints"] });
  };
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <Button asChild variant="ghost" className="mb-4"><Link to="/officer/complaints"><ArrowLeft className="size-4" /> Back</Link></Button>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="p-6">
              ...
              {c.images?.length > 0 && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-4">
                  {c.images.map((src, i) => <img key={i} src={src} alt={`Complaint image ${i + 1}`} className="aspect-video object-cover rounded-md border border-border" />)}
                </div>
              )}
          ...
          {c.resolutionImages?.length > 0 && (
            <Card><CardHeader><CardTitle className="text-base">Resolution proof</CardTitle></CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-2">
                  {c.resolutionImages.map((src, i) => <img key={i} src={src} alt={`Resolution proof ${i + 1}`} className="rounded-md border border-border" />)}
                </div>
              </CardContent>
            </Card>
          )}
